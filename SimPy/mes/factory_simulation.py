...
def start_lot(env, lot, factory):
    ...
    GlobalVariables.lot_status = pd.concat([GlobalVariables.lot_status,
                                 pd.DataFrame({'lot_id':current_lot_id, 'step_name':step_list[0], 'step_arrival_time':env.now},index=[current_lot_id])])
    ...
    GlobalVariables.lot_status = pd.concat([GlobalVariables.lot_status,
                                    pd.DataFrame({'lot_id':current_lot_id, 'step_name':step_list[1], 'step_arrival_time':env.now},index=[current_lot_id])])
    ...
    GlobalVariables.lot_status = pd.concat([GlobalVariables.lot_status,
                                    pd.DataFrame({'lot_id':current_lot_id, 'step_name':step_list[2], 'step_arrival_time':env.now},index=[current_lot_id])])
